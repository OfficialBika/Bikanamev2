...
def is_blocked_source(message: Message | None) -> bool:
    ...
    blocked_usernames = set(getattr(settings, "blocked_source_usernames", set()) or set())
    blocked_titles = [_clean(x) for x in (getattr(settings, "blocked_source_titles", []) or []) if _clean(x)]
    ...
    if chat is not None:
        ...
        if _norm_username(getattr(chat, "username", None)) in blocked_usernames:
            return True
        title = _clean(getattr(chat, "title", None))
        if title and title in blocked_titles:
            return True
    ...
    for user in _source_user_candidates(message):
        ...
        if _clean(full_name) in blocked_titles:
            return True

    origin = getattr(message, "forward_origin", None)
    hidden_name = _clean(getattr(origin, "sender_user_name", None) if origin else None)
    return bool(hidden_name and hidden_name in blocked_titles)
